...
impl IndexTablePrinter {
    pub fn print<T: Hash + Display, U: Display + Hash + Ord + Eq>(&self, index_table: &IndexTable<T, U>, path: &str) {
        match self {
...
fn write_index_table_text<T: Hash + Display, U: Display + Hash + Ord + Eq>(index_table: &IndexTable<T, U>, path: &str) {
...
fn write_index_table_binary<T: Hash + Display, U: Display>(index_table: &IndexTable<T, U>, path: &str) {
    let mut file = std::fs::File::create(path).expect("Unable to create file");
    for (key, value) in index_table {
        let value_comma_separated = value.iter().map(|x| x.to_string()).collect::<Vec<String>>().join(",");
        let line = format!("{}\t{}\n", key.to_string(), value_comma_separated);
...
fn write_index_table_debug<T: Hash + Display, U: Display + Hash + Ord + Eq>(index_table: &IndexTable<T, U>, path: &str) {
    let mut file = std::fs::File::create(path).expect("Unable to create file");
