...
mod parser_tests {
    ...
    fn test_parse_line_success() {
        let line = "ATOM      1  N   ALA A 340      -2.311   2.993 -33.448  1.00  6.00           N  ".to_string();
    ...
    fn test_parse_line_float() {
        // Error in X
        let line = "ATOM      1  N   ALA A   1      1A.000  10.000  10.000  1.00  0.00           N  ".to_string();
        let atom = parse_line(&line);
        assert!(atom.is_err());
        // Error in Y
        let line = "ATOM      1  N   ALA A   1      10.000  1A.000  10.000  1.00  0.00           N  ".to_string();
        let atom = parse_line(&line);
        assert!(atom.is_err());
        // Error in Z
        let line = "ATOM      1  N   ALA A   1      10.000  10.000  1A.000  1.00  0.00           N  ".to_string();
        let atom = parse_line(&line);
        assert!(atom.is_err());
        // Error in B-factor
        let line = "ATOM      1  N   ALA A   1      10.000  10.000  10.000  1A.00  0.00           N  ".to_string();
        let atom = parse_line(&line);
        assert!(atom.is_err());
    }
    ...
    fn test_parse_line_fail_serial() {
        ...
        let line = "ATOM      1  N   ALA A 3A0      -2.311   2.993 -33.448  1.00  6.00           N  ".to_string();
        let atom = parse_line(&line);
        assert!(atom.is_err());
        // Error in atom serial
        let line = "ATOM      K  N   ALA A   1      10.000  10.000  10.000  1.00  0.00           N  ".to_string();
        let atom = parse_line(&line);
        assert!(atom.is_err());
    }
}